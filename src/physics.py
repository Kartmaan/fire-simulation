...
# Heat limit values
MAX_TEMP = 2138 # Adiabatic flame temperature at constant pressure of gasoline
MIN_TEMP = 20 # Ambiant temperature
OXYGEN_CONSUMPTION_FACTOR = 10.0 # oxygen_consumed = fuel_consumed * OXYGEN_CONSUMPTION_FACTOR
MIN_OXYGEN_RATE = 5.0 # Minimum oxygen level below which combustion stops

# The 'combustion_heat' and 'thermal_capacity' properties in MaterialProperties use the units MJ and KJ, whereas
# thermodynamic formulas use the joule (J) as defined by the International System of Units. These constants will
# therefore be used to convert these values into joules. These conversion constants will be useful in the
# 'update_combustion' function.
...
def update_combustion(temperature_grid, fuel_grid, oxygen_grid, is_burning_grid, burn_rate_grid, combustion_heat_grid,
    ...
    #
    heat_generated = fuel_consumed * combustion_heat_grid * MEGAJOULES_TO_JOULES
    ...
    delta_temp = heat_generated / (cell_mass * thermal_capacity_grid * KILOJOULES_TO_JOULES)
    ...
    oxygen_consumed = fuel_consumed * OXYGEN_CONSUMPTION_FACTOR
    ...
    is_burning_grid &= (oxygen_grid > MIN_OXYGEN_RATE) & (fuel_grid > 0)
    ...
    burned_grid = burned_grid | (~is_burning_grid & (oxygen_grid <= MIN_OXYGEN_RATE))
